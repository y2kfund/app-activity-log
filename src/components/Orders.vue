...
// Fetch comment counts when orders change
watch(() => props.orders, (orders) => {
  if (orders && orders.length > 0) {
    const activityIds = orders.map(t => t.id)
    fetchCommentCounts(activityIds, 'order')
  }
}, { immediate: true })
...
const filteredActivities = computed(() => {
  ...
  return props.orders.filter(activity => {
...
const formatDate = (ordertime: string) => {
  if (!ordertime) return ''
  const [datePart, timePart] = ordertime.split(';')
  if (!datePart || !timePart) return ordertime
  ...
  if (isNaN(dateObj.getTime())) return ordertime
  ...
  <div class="positions-container">
    <div class="filter-section">
      ...
      <span v-if="!loading" class="result-count">
        {{ filteredActivities.length }} of {{ orders.length }} orders
      </span>
    </div>
    ...
    <div v-else-if="filteredActivities.length === 0" class="empty">
      ...
      <p>{{ filterText ? 'No orders match your filter' : 'No orders found' }}</p>
    </div>
